...
vector<QScrollArea*> GLWidget::settingsUi()
{
	...
	for(int i =0; i < (int)graphs.size(); ++i) {
		...
		if(tab != NULL) tabs.insert(tabs.begin(),tab);
...
{
	QBitmap pic = QBitmap(":/zoomIn.png"); if(pic.isNull()) ui->print("Warning: Null pic");
	QBitmap mask = QBitmap(":/zoomIn-mask.png"); if(mask.isNull()) ui->print("Warning: Null mask");    
    QBitmap pic2 = QBitmap(":/zoomOut.png"); if(pic2.isNull()) ui->print("Warning: Null pic2");
    
	zoomInCursor = QCursor( pic, mask);
    zoomOutCursor = QCursor( pic2, mask);  
}

void GLWidget::reportOnFinish(){ ui->print("The Editing has Finished"); }
